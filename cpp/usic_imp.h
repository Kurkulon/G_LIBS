#if !defined(USIC_IMP_H__25_09_2025__19_21) && !defined(__ADSPBF59x__)
#define USIC_IMP_H__25_09_2025__19_21

#pragma once

#include "usic.h"

u32 		USIC::_busy_mask = 0;
u32 		USIC::_alloc_mask = 0;
u32 		USIC::_req_reset_mask = 0;

USIC*		USIC::_usic[USIC_NUM] = {0};

#ifdef CPU_SAME53

	#if USIC_NUM == 8
		const byte	USIC::_usic_pid[USIC_NUM]		= {	PID_SERCOM0,		PID_SERCOM1,		PID_SERCOM2,		PID_SERCOM3,		PID_SERCOM4,		PID_SERCOM5,		PID_SERCOM6,		PID_SERCOM7 		};
		USICHWT	const USIC::_usic_hw[USIC_NUM]		= { HW::I2C0,			HW::I2C1,			HW::I2C2,			HW::I2C3,			HW::I2C4,			HW::I2C5,			HW::I2C6,			HW::I2C7			};
		const byte	USIC::_usic_gclk[USIC_NUM]		= {	GCLK_SERCOM0_CORE,	GCLK_SERCOM1_CORE,	GCLK_SERCOM2_CORE,	GCLK_SERCOM3_CORE,	GCLK_SERCOM4_CORE,	GCLK_SERCOM5_CORE,	GCLK_SERCOM6_CORE,	GCLK_SERCOM7_CORE	};
	#else
		const byte	USIC::_usic_pid[USIC_NUM]		= {	PID_SERCOM0,		PID_SERCOM1,		PID_SERCOM2,		PID_SERCOM3,		PID_SERCOM4,		PID_SERCOM5			};
		USICHWT	const USIC::_usic_hw[USIC_NUM]		= { HW::I2C0,			HW::I2C1,			HW::I2C2,			HW::I2C3,			HW::I2C4,			HW::I2C5			};
		const byte	USIC::_usic_gclk[USIC_NUM]		= {	GCLK_SERCOM0_CORE,	GCLK_SERCOM1_CORE,	GCLK_SERCOM2_CORE,	GCLK_SERCOM3_CORE,	GCLK_SERCOM4_CORE,	GCLK_SERCOM5_CORE	};
	#endif

#elif defined(CPU_SAM4SA)

const byte	USIC::_usic_pid[USIC_NUM]		= {	HW::PID::SPI_I,		HW::PID::TWI0_I,	HW::PID::TWI1_I,	HW::PID::USART0_I,	HW::PID::USART1_I,	HW::PID::UART0_I,	HW::PID::UART1_I	};
const USICHWT USIC::_usic_hw[USIC_NUM]		= {	HW::USIC0,			HW::USIC1,			HW::USIC2,			HW::USIC3,			HW::USIC4,			HW::USIC5,			HW::USIC6		};

#elif defined(CPU_XMC48)

const byte	USIC::_usic_pid[USIC_NUM]		= {	PID_USIC0,		PID_USIC0,		PID_USIC1,		PID_USIC1,		PID_USIC2,		PID_USIC2		};
USICHWT		const USIC::_usic_hw[USIC_NUM]	= {	HW::USIC0_CH0,	HW::USIC0_CH1,	HW::USIC1_CH0,	HW::USIC1_CH1,	HW::USIC2_CH0,	HW::USIC2_CH1	};

#elif defined(CPU_LPC824)

const byte	USIC::_usic_pid[USIC_NUM]		= {	HW::CLK::UART0_I,	HW::CLK::UART1_I,	HW::CLK::UART2_I,	HW::CLK::SPI0_I,	HW::CLK::SPI1_I,	HW::CLK::I2C0_I,	HW::CLK::I2C1_I,	HW::CLK::I2C2_I,	HW::CLK::I2C3_I	};
USICHWT		const USIC::_usic_hw[USIC_NUM]	= {	HW::USART0,			HW::USART1,			HW::USART2,			HW::SPI0,			HW::SPI1,			HW::I2C0,			HW::I2C1,			HW::I2C2,			HW::I2C3		};

#elif defined(CPU_LPC812)

const byte	USIC::_usic_pid[USIC_NUM]		= {	HW::CLK::UART0_I,	HW::CLK::UART1_I,	HW::CLK::UART2_I,	HW::CLK::SPI0_I,	HW::CLK::SPI1_I,	HW::CLK::I2C_I		};
USICHWT		const USIC::_usic_hw[USIC_NUM]	= {	HW::USART0,			HW::USART1,			HW::USART2,			HW::SPI0,			HW::SPI1,			HW::I2C				};

#elif defined(WIN32)

const byte	USIC::_usic_pid[USIC_NUM] = {0};

USICHWT	const USIC::_usic_hw[USIC_NUM] = { 0 };

#elif defined(__ADSPBF70x__)

const byte	USIC::_usic_pid[USIC_NUM]		= {	PID_UART0_STAT, PID_UART1_STAT };
USICHWT		const USIC::_usic_hw[USIC_NUM]	= {	HW::UART0, HW::UART1 };

#elif defined(__ADSPBF60x__)

const byte	USIC::_usic_pid[USIC_NUM]		= {	PID_UART0_STAT, PID_UART1_STAT };
USICHWT		const USIC::_usic_hw[USIC_NUM]	= {	HW::UART0, HW::UART1 };

#endif 

//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

bool USIC::Usic_Connect()
{
	if (_usic_num >= USIC_NUM || (_alloc_mask & _usic_mask) || _usic[_usic_num] != 0) return false;

	_alloc_mask |= _usic_mask;

	_usic[_usic_num] = this;
	_uhw = _usic_hw[_usic_num];

#ifdef CPU_SAME53

	_ugclk = _usic_gclk[_usic_num];

#endif

	return true;
}

//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

void USIC::Usic_Disconnect()
{
	_alloc_mask &= ~_usic_mask;
	_usic[_usic_num] = 0;
}

//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

#if defined(CPU_SAME53) || defined(CPU_SAM4SA)

//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

void USIC::Usic_Update()
{
	if ((_req_reset_mask & _usic_mask) && !(_busy_mask & _usic_mask))
	{
		InitHW();
	};
}

//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

#elif defined(CPU_XMC48)

//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

void USIC::Usic_Update()
{
	if (_req_reset_mask & _usic_mask)
	{
		byte num = _usic_num ^ 1;

		u32 mask = (1UL<<num);

		_req_reset_mask |= mask;

		if (!(_busy_mask & (mask|_usic_mask)))
		{
			HW::Peripheral_Disable(_upid);

			InitHW();

			if ((_alloc_mask & mask) && _usic[num] != 0)
			{
				HW::Peripheral_Disable(_usic[num]->_upid);

				_usic[num]->InitHW();
			};
			
			_req_reset_mask &= ~(mask|_usic_mask);
		};
	};
}

//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

#endif



#endif // #if !defined(USIC_IMP_H__25_09_2025__19_21) && !defined(__ADSPBF59x__)

