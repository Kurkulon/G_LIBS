...
bool S_SPIM::Connect(u32 baudrate)
{
	...
	if (baudrate == 0) baudrate = 1;
		...
		if (baud > SPI_SCBR(~0)) baud = SPI_SCBR(~0);
...
void S_SPIM::WritePIO(const void *data, u16 count)
{
	...
	while (count != 0)
	{
		_uhw->spi.TDR = *(p++); count--;

		while((_uhw->spi.SR & SPI_TDRE) == 0);
	};

	while((_uhw->spi.SR & SPI_TXEMPTY) == 0) HW::ReadMem32(&_uhw->spi.RDR);
...
void S_SPIM::ReadPIO(void *data, u16 count)
{
	...
	while (count != 0)
	{
		...
		while((_uhw->spi.SR & SPI_RDRF) == 0);

		*(p++) = _uhw->spi.RDR; count--;
	};

	_uhw->spi.CR = SPI_SPIDIS;
...
void S_SPIM::ReadAsyncDMA(void *data, u16 count)
{
	...
	_dma.ReadPeripheral(data, count, 0, 0);
	_dma.WritePeripheral(data, count, 0, 0);
			...
			else
			{
				...
				if (_dsc != 0)
				{
					...
					if (dsc.alen == 0)
					{
						if (dsc.wdata != 0 && dsc.wlen > 0)
						{
							spi.MR = _MR & ~SPI_WDRBT;

							_dma.WritePeripheral(dsc.wdata, dsc.wlen, 0, 0);

							_state = WRITE; 
						}
						else if (dsc.rdata != 0 && dsc.rlen > 0)
						{
							while(spi.SR & SPI_RDRF) HW::ReadMem32(&spi.RDR);

							spi.MR = _MR|SPI_WDRBT;

							_dma.ReadPeripheral(dsc.rdata, dsc.rlen, 0, 0);
							_dma.WritePeripheral(dsc.rdata, dsc.rlen, 0, 0);
					...
					else
					{
						...
						_dma.WritePeripheral(&dsc.adr, dsc.alen, dsc.wdata, dsc.wlen);
